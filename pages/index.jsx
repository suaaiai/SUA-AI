          ...
          body: JSON.stringify({
            ...
            messages: [{
              ...
              content: [
                ...
                {
                  type: "text",
                  text: `이 영수증 이미지에서 식재료 품목을 추출해주세요.
반드시 아래 JSON 형식만 반환하세요. 다른 텍스트는 절대 포함하지 마세요:
{
  "items": [
    { "name": "품목명", "quantity": 숫자, "unit": "단위(g/ml/개/kg/L 중 하나)", "price": 숫자 }
  ]
}
- 단위는 반드시 g, ml, 개, kg, L 중 하나로 추정해주세요
- 가격은 숫자만 (원 제외)
- 수량이 불명확하면 1로 설정
- 영수증이 아니거나 식재료가 없으면 {"items": []} 반환`
                }
              ]
            }]
          })
      ...
      {showOcrModal && (
        <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 998, display: "flex", alignItems: "flex-end" }} onClick={() => { if (!ocrLoading) { setShowOcrModal(false); setOcrPreview(null); setOcrResult(null); } }}>
          <div style={{ background: "#1a1a2e", border: "1px solid #6366f140", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", width: "100%", maxHeight: "85vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <h2 style={{ fontSize: 17, fontWeight: 800, marginBottom: 4 }}>📷 영수증 분석 중...</h2>
            <p style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>AI가 식재료 품목을 자동으로 추출하고 있어요</p>
